...
int ssd1306_get_font(uint8_t character)
{
  if (character >= 'A' && character <= 'Z') {
    ...
    return 37; // O índice do ponto
  }
  else if (character == '%') {
    return 38; // O índice da porcentagem
  }
  else if (character == 0xB0) { // 0xB0 é o código ASCII/UTF-8 para o símbolo de grau '°'
    return 39; // O índice do símbolo de grau
  }
  else
    return 0; // Para qualquer outro caractere não definido, retorna o blank space
}
